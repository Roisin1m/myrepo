...
S_matrix <- sqrt(sum(res^2) / length(y)- ncol(X))
...
hessian_result <- optim(par = initial_par, fn = nll_lm, data = df, method = "L-BFGS-B", 
                        lower = c(rep(-Inf, length(initial_beta)), 0.00001), 
                        upper = c(rep(Inf, length(initial_beta)), Inf), hessian = TRUE)

hessian <- hessian_result$hessian
...
beta_hat1 <- coef(lm_fit)
beta_hat1

sigma_hat1 <- summary(lm_fit)$sigma
sigma_hat1
